...
export const RepoSection = ({ visible, onClose }: RepoSectionProps) => {
  ...
  const handleRepoSubmit = () => {
    if (editingRepo) {
      chatStore.updateRepo(editingRepo, repoInput)
      setEditingRepo(null)
    } else {
      chatStore.addRepo(repoInput)
    }
    ...
    >
      <View style={[baseStyles.modalContainer, styles.container]}>
        ...
        <View style={styles.section}>
          ...
          {chatStore.repos.map((repo) => (
            <View key={`${repo.owner}/${repo.name}/${repo.branch}`} style={styles.repoItem}>
              <TouchableOpacity
                style={[
                  styles.repoButton,
                  chatStore.activeRepo === repo && styles.buttonActive
                ]}
                onPress={() => {
                  handleEditRepo(repo)
                  chatStore.setActiveRepo(repo)
                }}
              >
              ...
              <TouchableOpacity
                ...
                onPress={() => chatStore.removeRepo(repo)}
              >
